document.addEventListener("DOMContentLoaded", () => {
  ...
  let chocolateGrid = Array(ROWS)
    .fill()
    .map(() => Array(COLS).fill(false)); // false means not eaten
  ...
  wrapper.addEventListener(
    ...
    (e) => {
      ...
      if (
        ...
        !wrapper.classList.contains("unwrapped")
      ) {
        unwrapChocolate();
      }
    },
  ...
  // Old unwrap sound function removed, handled by unified synthesizer above
  ...
  function playSound(type) {
    ...
    if (type === "eat") {
      ...
      // Rustle sound (noise buffer would be better, but using simple osc for now)
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLS; col++) {
      ...
      // Center piece check removed - all pieces are normal now
